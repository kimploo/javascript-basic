...
// 함수형 프로그래밍
// -> 가능하면 외부 변수를 사용하지 않는게 좋다 -> 이 함수가 실행되었을때 어떤 외부에 무언가를 변경할 수 있으면 좋은 코드가 아니다.
const idCheck = (event) => {
  // event 객체 -> userId 주소값이 있다.
  // 아이디 유효성 검사
  const userId = event.target;
  const userIdValue = userId.value.trim();
  const regexId = /^(?!^[0-9])[a-zA-Z0-9]{8,16}$/; // 영문+숫자 8~16자 정규식 -> ChatGPT가 잘 짜줌
...
cerfRequestBtn.addEventListener(
  "click",
  () => {
...
function updateAllCheck() {
  // 개별 체크박스중 단 한개라도 unchecked가 되면 전체 체크박스는 unchecked가 됨
...
termsService.addEventListener("change", (e) => {
  // [필수]이용약관
  isTrueTermsService = e.target.checked;
});

termsPrivacy.addEventListener("change", (e) => {
  // [필수]개인정보 수집 및 이용
...
userNick.addEventListener("input", () => {
  nickCheck();
}); // nickname 입력 이벤트
...
userPhoneCerf.addEventListener("input", () => {
  phoneCerfCheck();
}); // phone 인증번호 입력 이벤트

doubleCheckBtnId.addEventListener("click", () => {
  handleIdDoubleCheck();
}); // id 중복확인 이벤트
doubleCheckBtnNick.addEventListener("click", () => {
  handleNickDoubleCheck();
}); // nickname 중복확인 이벤트
